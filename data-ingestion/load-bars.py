'''
Description:
This script pulls minute and day bars for inputted ticker symbols for an
inputted date range and inserts the data into corresponding PostgreSQL tables
in RDS.

Usage:
$ python load-bars.py -t/--ticker NVDA,INTC -s/--start 20220314 -e/--end 20220420
...
def main() -> int:
    ...
    PG_TABLE_MINUTE = 'bars_minute_eastern'
    PG_TABLE_DAY = 'bars_day'
    ...
    while current <= end:
        ...
        # pull day bars for tickers for day
        print(f'[ INFO ] Fetching day bars for {tickers} on {current}')
        request_params = StockBarsRequest(symbol_or_symbols=tickers, timeframe=TimeFrame.Day, start=(start_dt - datetime.timedelta(days=1)), end=start_dt)
        stock_bars_day = stock_client.get_stock_bars(request_params)

        # convert alpaca data to DataFrame
        stock_bars_minute_df = stock_bars_minute.df
        stock_bars_minute_df.reset_index(inplace=True)
        stock_bars_day_df = stock_bars_day.df
        stock_bars_day_df.reset_index(inplace=True)

        # drop unused columns
        stock_bars_minute_df.drop('trade_count', axis=1, inplace=True)
        stock_bars_minute_df.drop('vwap', axis=1, inplace=True)
        stock_bars_day_df.drop('trade_count', axis=1, inplace=True)
        stock_bars_day_df.drop('vwap', axis=1, inplace=True)
        ...
        # day dataframe to only use date instead of datetime
        stock_bars_day_df['date'] = pd.to_datetime(stock_bars_day_df['timestamp']).dt.date
        stock_bars_day_df.drop('timestamp', axis=1, inplace=True)

        # insert rows and commit
        stock_bars_minute_df.to_sql(name=PG_TABLE_MINUTE, con=conn, if_exists='append', index=False)
        stock_bars_day_df.to_sql(name=PG_TABLE_DAY, con=conn, if_exists='append', index=False)
        conn.commit()
